...
export const MAX_LAT = 85.05112878; // Mercator limit in degrees
const PI = Math.PI;
...
function lonToX(lonDeg: number) { return R * (normalizeLon(lonDeg) * PI / 180); }
function xToLon(x: number) { return normalizeLon((x / R) * 180 / PI); }
function latToY(latDeg: number) {
 ...
 */
export function getDigiPin(lat: number, lon: number, levels = 10): string {
    ...
    for (let level = 1; level <= levels; level++) {
        ...
        const rowRaw = 5 - Math.floor((y - minY) / yDiv); // top row = 0
...
export function getLatLngFromDigiPin(digiPin: string): { latitude: number; longitude: number } {
    ...
    for (let i = 0; i < pin.length; i++) {
        ...
        outer: for (let r = 0; r < 6; r++) {
            for (let c = 0; c < 6; c++) {
                if (DIGIPIN_GRID[r][c] === ch) { ri = r; ci = c; break outer; }
            }
        }
...
export function approxCellSizeMeters(levels: number): number {
